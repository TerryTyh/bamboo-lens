...
- `scripts/build_decision_deposition.py`
  把正式事件的业务/财务/估值影响转成公司主页回写计划
- `scripts/build_company_page_overrides.py`
  把回写计划转成公司主页可读取的动态覆盖层，先自动更新当前结论区
...
- 已加入 V2 决策沉淀层：正式事件会生成“应该回写到公司主页哪个板块”的计划，避免内容只停留在事件详情页
- 已加入公司主页动态覆盖层：最新高质量事件可以先自动更新公司主页“当前结论”区，后续再人工精修完整档案
