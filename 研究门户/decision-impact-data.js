window.BAMBOO_LENS_DECISION_IMPACT = {
  ...
  "items": [
    ...
    },
    {
      ...
      "decision_change": "这是一条 P2 级平台外延事件。它真正改变的不是 NVIDIA 又多了一个合作伙伴，而是公司开始进入消费级 AI 功能背后的高隐私云端推理链条。对 NVIDIA 来说，这意味着 Blackwell 的价值不再只由训练吞吐或通用推理决定，机密计算、云端部署和可信执行环境也开始成为平台能力的一部分。没有改变的是，原文仍缺少订单规模、部署范围和商业化口径，所以它更",
      "business_impact": "业务层面，这条进展强化了三件事。第一，NVIDIA 开始切入高隐私要求的消费级云端推理入口，说明 Apple Intelligence 这类面向海量终端用户的功能，未来可能不只依赖端侧芯片，还需要可信的云端推理底座。第二，合作链条把 Apple、Google Cloud 和 NVIDIA 连在一起，意味着 NVIDIA 既可能受益于云厂商基础设施采购，也可能受益于更多面向终端产品的 server-side inference 扩容。第三",
