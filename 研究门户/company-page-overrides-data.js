window.BAMBOO_LENS_COMPANY_PAGE_OVERRIDES = {
  ...
  "source_deposition_at": "2026-06-16T06:34:02",
  "companies": {
    "tsmc": {
    ...
    "nvidia": {
      ...
      "businessImpact": "业务层面，这条事件至少影响三条线。第一条是 Data Center GPU 需求：Apple Intelligence 如果持续扩大功能范围，服务端推理会变成长期负载，而不是发布会阶段的一次性项目。第二条是机密计算与系统软件 attach：当 Apple 这类平台型客户要求把 GPU 放进可验证隐私架构里，NVIDIA 的卖点就不再只是训练/推理性能，还包括安全隔离、远程度量和可信运行能力。第三条是云生态协同：Apple 把 PCC 扩到 Google Cloud，意味着 NVIDIA 在超大客户架构中的角色从“云厂商 GPU 供应商”进一步延伸到上层消费 AI 产品的后端底座。",
      "valuationImpact": "估值和动作上，这条事件支持继续给 NVIDIA 平台溢价，尤其是对推理基础设施和系统级能力的溢价。正面在于，合作对象同时覆盖 Apple 与 Google，且原文把应用场景、GPU 代际和安全机制都写得比较具体，说明机密计算已经进入真实生产工作负载。限制在于，公告没有披露 GPU 数量、合同金额、推理负载规模或软件收入口径，因此它更适合作为“平台能力被高质量客户验证”的证据，而不是直接上调短期盈利预测。动作上维持 A 池核心，并把后续机密计算是否复制到更多企业与消费级推理场景列为重点观察。",
      "nextCheck": "跟踪 Apple 后续是否进一步披露 Private Cloud Compute 的部署范围、Google Cloud 占比和 Apple Intelligence 新功能上线节奏，确认这不是单点安全宣传。；观察 NVIDIA 财报或电话会是否开始明确提到 confidential inference、Confidential Computing attach rate 或安全型推理工作负载成为新增需求来源。",
      ...
      "sectionDeposits": {
        ...
        "businessMap": {
          "segments": [
            {
              "key": "business-segment-财报-经营验证",
              "title": "财报/经营验证｜Apple 把 Private Cloud Compute 扩到 Google Cloud，NVIDIA Blackwell 开始切入隐私推理底座",
              "scale": "2026-06-09｜机密计算 / 云推理基础设施 / Apple Intelligence",
              "text": "业务层面，这条事件至少影响三条线。第一条是 Data Center GPU 需求：Apple Intelligence 如果持续扩大功能范围，服务端推理会变成长期负载，而不是发布会阶段的一次性项目。第二条是机密计算与系统软件 attach：当 Apple 这类平台型客户要求把 GPU 放进可验证隐私架构里，NVIDIA 的卖点就不再只是训练/推理性能，还包括安全隔离、远程度量和可信运行能力。第三条是云生态协同：Apple 把 PCC 扩到 Google Cloud，意味着 NVIDIA 在超大客户架构中的角色从“云厂商 GPU 供应商”进一步延伸到上层消费 AI 产品的后端底座。 原文要点：这篇原文讲的不是泛泛而谈的安全宣传，而是 Apple 已经把 NVIDIA 的机密计算 GPU 用到 Private Cloud Compute（PCC）里，并把原本主要在 Apple 自有数据中心运行的服务端推理，扩展到 Google Cloud。文章明确说，Apple Foundation Models 的部分服务端推理将使用带机密计算能力的 NVIDIA GPU，支撑下一代 Apple Intelligence 功能。；原文同时解释了这套架构为什么重要：NVIDIA Blackwell GPU 的 Confidential Computing 会被嵌入 PCC 的硬件安全架构里，在 Google Cloud 上为敏感推理工作负载建立 trusted execution environment。系统会在数据进入服务器前做加密验证，确保底层基础设施…"
            }
          ],
          "moat": [
            {
              "key": "business-moat-财报-经营验证",
              "title": "护城河/业务主线是否变化｜财报/经营验证",
              "text": "这是一条 P1 级平台质量强化事件。真正的增量不是 Apple 多用了一家 GPU 供应商，而是 NVIDIA 把 Blackwell + Confidential Computing 从“高端安全功能”推进成 Apple Intelligence 后端推理的生产底座之一，并且落在 Google Cloud 这种外部云环境里。它说明高性能推理、云端弹性和隐私隔离正在开始绑定销售，NVIDIA 在 AI 基础设施中的价值主张也从纯算力继续扩到可信执行与安全治理。"
            }
          ]
        },
        "valuationModel": {
      ...
      "depositEvents": [
        {
          "eventIndex": 0,
          "title": "Apple 把 Private Cloud Compute 扩到 Google Cloud，NVIDIA Blackwell 开始切入隐私推理底座",
          "date": "2026-06-09",
          "priority": "P1",
